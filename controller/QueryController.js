...
function queryInsert(req, res, tableName, entity, autogeneratedPropertys, triggerPreInsertInterceptors = true, blobProperty, processResult = true) {
    if (blobProperty === null) {
        blobProperty = undefined;
    }
    if (!autogeneratedPropertys) {
        autogeneratedPropertys = [];
    }
    if (triggerPreInsertInterceptors) {
    ...
    const r = db.promisedQuery(q, insertValues);

    if (processResult) {
        return r.then((rows) => {
            if (blobProperty !== undefined) {
                entity[blobProperty] = blob;
            }
            config.interceptors.postInsert.forEach(e => {
                e(req, tableName, entity, autogeneratedPropertys, blobProperty);
            });
...
function queryUpdate(req, res, tableName, entity, autogeneratedPropertys) {
    config.interceptors.preUpdate.forEach(e => {
        if (!(entity = e(req, tableName, entity, autogeneratedPropertys))) {
            res.status(403).send('not authorized');
            return;
        }
    });
...
function querySelectSingle(req, res, tableName, entity, autogeneratedPropertys, triggerPreSelectInterceptors = true, selectResultProcessor = processSelectResultAsRowset) {
